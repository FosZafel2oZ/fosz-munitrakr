...
Vanilla JS + CSS + Chart.js (vendored). No backend, no build step. All data lives in `localStorage`. Deployed at **https://fosz-munitrakr.pages.dev** (Cloudflare Pages, auto-deploys on push to `main`). Source: **https://github.com/FosZafel2oZ/fosz-munitrakr**. Current version: **v80**.
...
### Save as New
- Edit Record modal only: a green **Save as New** button (`.btn-alt`, `var(--in)`) sits between Delete and Save. Clicking it sets the one-shot `saveAsNew` flag and resubmits the same form. The submit handler captures then immediately clears the flag at the top (`const asNew = saveAsNew; saveAsNew = false;`) so a failed validation never leaves it stuck — fixing a field and pressing "Save Record" updates as usual. When the save is `asNew`, the API call becomes a POST instead of a PUT, producing a fresh `id` / `createdAt`; `ruleId` is never carried over onto the duplicate. The two-pass color-confirmation detour (new category/sub needing colours) re-arms the flag (`saveAsNew = asNew`) before returning, so the as-new intent survives into the second submit. Duplicates are plain, independent records — the split-the-bill and paid-by-someone-else guards already gate on `!editingId`, so a Save-as-New duplicate can't accidentally re-trigger those Add-only flows.
...
- **Lockstep version bump on every release:** `APP_VERSION` in `app.js` AND `CACHE` in `sw.js` must match. Current: `v80` / `munitrakr-v80`.
...
| `saveAsNew` | one-shot flag: the next record-form submit saves a copy as a new record (Save as New button, Edit Record modal only) |
