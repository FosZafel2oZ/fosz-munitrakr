...
| `balanceAfterRecord(debts, recordId)` | pure (from `debts.js`) — the person's signed outstanding immediately after a given record: `balanceBefore` (called WITHOUT a people map, so a since-deleted person's history still counts) plus that record's own signed amount; used by `shareDebtStatement` for the footer's `balanceAfter` |
